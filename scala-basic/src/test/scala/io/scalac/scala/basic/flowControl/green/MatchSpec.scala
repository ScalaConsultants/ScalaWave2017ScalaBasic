...
class MatchSpec extends WordSpec with MustMatchers {
  ...
  def classifyHttpStatus(status: Int): String = {
    status match {
      case _ if status >= 100 && status <= 199 =>
        "INFORMATION"
      case _ if status >= 200 && status <= 299 =>
        "SUCCESS"
      case _ if status >= 300 && status <= 399 =>
        "REDIRECT"
      case _ if status >= 400 && status <= 499 =>
        "CLIENT_ERROR"
      case _ =>
        "SERVER_ERROR"
    }
  ...
  "FlowControl.If" should {
    ...
    "classify 500-599 responses as SERVER_ERROR" in {
      ...
      classifyHttpStatus(999) mustBe "SERVER_ERROR"
    }
  }
}
